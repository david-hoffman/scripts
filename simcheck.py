...
if __name__=='__main__': #check to see if we're being run from the command line
    ...
    ft_data = ifftshift(fftn(fftshift(data, axes=(1,2)), axes=(1,2)), axes = (1,2))
    #average only along phase, **not** orientation
    #This should be the equivalent of the FT of the average image per each phase
    #(i.e it should be symmetric as the phases will have averaged out)
    ft_data_avg = ft_data.reshape(newshape).mean(1)
    #Do the same, but take the absolute value before averaging, in this case the
    #signal should add up because the phase has been removed
    ...
    if arg['--raw']:
        fig, ax = display_grid({k : v for k,v in zip(['Phase {}, Orientation {}'.format(i,j) for i in range(nphases) for j in range(norients)], data)},
